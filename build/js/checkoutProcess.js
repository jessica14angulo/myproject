...
export default class T {
  ...
  displayTotal() {
    (document.querySelector("#shipping").innerHTML = `$${parseFloat(
      ...
      (document.querySelector("#tax").innerHTML = `$${parseFloat(
        this.tax
      ).toFixed(2)}`),
    ...
    return u(this, null, function* () {
      ...
      try {
        ...
        d(this.key, []),
          ...
          window.location.assign(`../checkedout/?orderID=${n.orderId}`);
      } catch (i) {
  ...
  prepareTemplate(e, t) {
    return (
      (e.querySelector(".error-type").innerHTML = t),
      (e.querySelector(".alertparenttemplate").id = t.replace(/\s/g, "")),
      e.querySelector("#error-button").classList.add(t.replace(/\s/g, "")),
      e
    );
  }
}
